...
def r2hpl(r_list, height, width,class_id):   # list   -6.07 103.44 121.07 102.87 121.44 184.98 -5.70 185.55 
    min_x = 10000
    max_x = 0
    min_y = 10000
    max_y = 0
    ...
    list_p = []    
    list_l = []                
    if not (min_x ==0 or min_y == 0 or max_x == width-1 or max_y == height-1):
        list_p.append([(min_x+max_x)/2, (min_y+max_y)/2])   
        list_l.append(1)

    coords = np.array(r_list[:8], dtype=np.float32).reshape(4, 2)
    ...
    for corner in coords:
        ...
        # 约束在图像范围内
        if new_point[0]<=0 or new_point[0]>=width-1:
            continue
        if new_point[1]<=0 or new_point[1]>=height-1:
            continue
        new_point = list(new_point)
        neg_points.append(new_point)
        neg_labels.append(0)
    list_p.extend(neg_points)
    list_l.extend(neg_labels)
    result_p = np.array(list_p)
    result_l = np.array(list_l)
    # if class_id in [2,4,]:
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:   # 读取1.txt，即图片下面的所有检测结果 
                    ...
                    for line in f:  # 60.53 234.26 121.40 212.64 127.80 230.63 66.92 252.25 7
                        ...
                        result_h, result_p, result_l, det_bbox = r2hpl(det_rbox, height, width,class_id)
                        # 融合所有的结果，作为array输入
...
if __name__ == "__main__":
    ...
    submit_path = "instances/test/2_result.json"
