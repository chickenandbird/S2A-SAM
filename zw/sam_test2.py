...
def main():
    ...
    use_point = False
    # ===== 配置参数 =====
    IMAGE_PATH_HEAD = f"instances2/test/images"  # 替换为您的图像路径
    DOTA_TXT_PATH_HEAD = f"instances2/test/annfiles2/"  # 替换为DOTA标注文件路径
    ...
    for filename in os.listdir(IMAGE_PATH_HEAD):
        ...
        if all_masks:
            ...
            plt.savefig(f"instances2/test/images_seg/{filename.split('.')[0]}.jpg", bbox_inches='tight', pad_inches=0, dpi=300)#保存图像的路径
            # plt.show()
            plt.close() 
